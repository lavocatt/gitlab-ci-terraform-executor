...
resource "aws_iam_role" "vmimport_aoc" {
  ...
  tags = merge(
    var.imagebuilder_tags, { Name = "Image Builder vmimport role for aoc" },
  )
}
...
resource "aws_iam_policy" "spotfleet_iam_spot_policy_aoc" {
  name   = "spotfleet_iam_spot_policy_aoc"
...
resource "aws_iam_role" "spot_fleet_tagging_role_aoc" {
  ...
  name               = "imagebuilder_spot_fleet_role_aoc"
}
...
resource "aws_s3_bucket" "imagebuilder_s3_aoc" {
  ...
  tags = merge(
    var.imagebuilder_tags, { Name = "Image Builder S3 AOC bucket" },
  )
}
...
resource "aws_iam_policy" "imagebuilder_workers_aoc_s3" {
  name   = "imagebuilder_workers_aoc_s3"
  policy = data.aws_iam_policy_document.imagebuilder_s3_aoc.json
}

# Create the IAM user.
resource "aws_iam_user" "imagebuilder_worker_aoc" {
  name = "imagebuilder_worker_aoc"

  tags = merge(
    var.imagebuilder_tags, { Name = "Image Builder AOC Worker" },
  )
}
